...
def ft_tqdm(lst: range) -> None:
    ...
    length = len(lst)
    lengt_digits = len(str(length))
    for e in lst:
        percentage = int(100 * ((e+1)/(length)))
        text = "{:3}%|".format(percentage)
        
        loading_bar_width_filled = int((DEFAULT_TERMINAL_WIDTH * percentage/100))
        text += '█' * loading_bar_width_filled

        loading_bar_width_empty = DEFAULT_TERMINAL_WIDTH - loading_bar_width_filled
